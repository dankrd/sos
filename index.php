...
<head>
  ...
  <title>S.O.S Ethos Soluciones de Software SA</title>
  ...
  <script src="j/functionGral.js"></script>
</head>
<body>
    <div id='headerIng'>
        <img id='logo' src="img/SOS-logo.png">
    <h1>Help Desk</h1>
    <h3>Inicie la Sesión Para Gestionar Los Servicios!</h3>
    </div>
    <section id='contenedor'>
        <form action="<?php echo $loginFormAction; ?>" method="POST" name='cont_ingreso' id='cont_ingreso'>
       		<input type='text' name='cUsuario' id='cUsuario' placeholder='Ingrese su Usuario' title='Coloque Aqui su Usuario'>
       		<input type='password' name='cContrasena' id='cContrasena' placeholder='Ingrese su Contraseña'  title='Coloque Aqui su Contraseña'>
          <input id="ingreso" name="ingreso" type="button" value="Ingresar a la Aplicación" title='Click'>
          <br>olvido su contraseña?
        </form>
    </section>
    <footer>
    ...
    <script >
        $('#cUsuario').focus();
        $('#contenedor').css('min-height',$(window).innerHeight()-$('footer').innerHeight()-$('#headerIng').innerHeight()-60);
